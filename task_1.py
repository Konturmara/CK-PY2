class Game:
    def __init__(self, name: str, cost: int):
        ...
        if name:  # проверяем, что существует значение в имени
    ...
    def update_info_selling(self) -> str:
        """
...
class VideoGame(Game):
    ...
    def is_ready_to_play(self) -> str:
        ...
        Примеры:
        >>> hades = VideoGame("Hades", 918, 'PC')
        >>> hades.is_ready_to_play()
        """
        platform_ready = f'{self.platform} включен.'
        game_ready = super().is_ready_to_play()
